...
public class ChangeService {
  ...
  public List<Method> fetchChangeSets(final Class<?> type) throws LiquiCouchChangeSetException {
  ...
  private List<Method> filterChangeSetAnnotation(List<Method> allMethods) throws LiquiCouchChangeSetException {
    ...
    for (final Method method : allMethods) {
      if (method.isAnnotationPresent(ChangeSet.class)) {
        ...
        if (changeSetIds.contains(id)) {
          throw new LiquiCouchChangeSetException(String.format("Duplicated changeset id found: '%s'", id));
        }
        changeSetIds.add(id);
